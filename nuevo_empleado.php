...
if (isset($_POST["btn_nuevo_empleado"]))
{
    $btn=$_POST["btn_nuevo_empleado"];
    if($btn == "Agregar Empleado")
    {
		...
		$query = "EXECUTE PROCEDURE SP_EMPLEADOS_CREATE('$fecha_nacimiento','$fecha_contratacion','$PN','$SN','$PA','$SA','$Direc_Referencia','$Direc_Calle','$Direc_Avenida','$Direc_Num_Casa','$Direc_Ciudad','$Direc_Depto','$Correo_2','SYSDBA','$Correo_1')";
		...
		echo "<script> alert('Un Empleado ha sido Almacenado en la Base de Datos');</script>";
		?>
		<script type="text/javascript">
			window.location.href="empleado.php" //CAMBIAR;
		</script>
		<?
...
<body>
	<form name="fe" action="nuevo_empleado.php" method="POST">
        ...
        <tr>
            <td align="center"><b>Fecha de Contratacion:  </b></td><td><label><input name="fecha_contratacion" type="date" size=35 value="<?php echo date("Y-m-d");?>" style="font-size:18px" required /></label></td>
        </tr>
        <tr>
        <!-- Primer Nombre -->
        <tr>
    	...
    	<input id='crear' type="submit" name="btn_nuevo_empleado" value="Agregar Empleado"/>
	</form>
</body>
</html>
